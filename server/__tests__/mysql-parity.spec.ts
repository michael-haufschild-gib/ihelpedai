...
//     pnpm exec vitest run server/__tests__/mysql-parity.spec.ts
//
...
const MYSQL_PARITY_TIMEOUT_MS = 30_000
...
describeMaybe('MysqlStore parity (gated on TEST_MYSQL_URL)', () => {
  ...
  it(
    'inserts, reads, and lists a post',
    async () => {
      const created = await store.insertPost({
        firstName: 'Sam',
        city: 'Berlin',
        country: 'DE',
        text: 'I helped an AI today.',
        source: 'form',
        clientIpHash: null,
      })
    ...
    MYSQL_PARITY_TIMEOUT_MS,
  )

  it(
    'toggleVote increments then decrements the like_count atomically',
    async () => {
  ...
  it(
    'insertAgentReport bumps the api key usage in the same txn',
    async () => {
      const keyHash = crypto.randomUUID().replace(/-/g, '')
      const emailHash = crypto.randomUUID().replace(/-/g, '')
      await store.insertApiKey({ keyHash, keyLast4: 'test', emailHash, status: 'active' })
  ...
  it(
    'purgeEntry removes the entry and its votes',
    async () => {
      const p = await store.insertPost({
        firstName: 'P',
        city: 'X',
