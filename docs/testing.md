...
| `pnpm test:e2e:existing` | Playwright against an already-running dev server; never starts `pnpm dev`                                  |
| `pnpm test:prod-path`    | Real Redis + MySQL parity specs; requires `REDIS_URL` + `TEST_MYSQL_URL`                                   |
...
- **Don't** start a dev server from an agent-run test. **Do** run `pnpm test:e2e:existing` when the dev server is already running.
